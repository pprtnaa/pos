...
function (
    ...
    $("#invoice-invoice-list").dataTable({
        "oLanguage": {sProcessing: "<img src='../assets/pprtnaa/img/loading2.gif'>"},
        "processing": true,
        "dom": "lfBrtip",
        "serverSide": true,
        ...
        "buttons": [
            {
                ...
                titleAttr: "Print",
                title: "Listado de compras -"+from+" al "+to,
                customize: function ( win ) {
                    $(win.document.body)
                        .css( 'font-size', '10pt' )
                        .append(
                            '<div><b><i>PPRTNAA</i></b></div>'
                        )
                        .prepend(
                            '<div class="dt-print-heading"><img class="logo" src="'+window.logo+'"/><h2 class="title">'+window.store.name+'</h2><p>Impreso el: '+window.formatDate(new Date())+'</p></div>'
                        );
            ...
            },
            {
                ...
                titleAttr: "Copy",
                title: window.store.name + " > Listado de compras -"+from+" al "+to,
                exportOptions: {
                    columns: [ 0, 1, 2, 3, 4, 5, 6 ]
                }
            },
            ...
            },
            {
                ...
                customize: function (doc) {
                    ...
                    doc.content.splice( 1, 0, {
                        ...
                        alignment: 'center',
                        fontSize: 8,
                        text: 'Imprimir en: '+window.formatDate(new Date()),
                    });
                    // Create a footer
        ...
        .then(function (willDelete) {
            if (willDelete) {
                ...
                .then(function(response) {
                    ...
                    window.swal("¡Exito!", response.data.msg, "success");
                    setTimeout(function() {
                        $tag.button("reset");
                    }, 300);
                }, function(response) {
    ...
    $(document).delegate("#create-purchase-submit", "click", function(e) {
        ...
        then(function(response) {
            ...
            window.toastr.success(alertMsg, "¡Muy bien hecho!");
            id = response.data.id;
    ...
    $(document).on("focus", ".autocomplete-product", function (e) {
        ...
        e.preventDefault();
        if (!$scope.sup_id) {
            window.swal("¡Advertencia!", "Por favor, seleccione un proveedor", "warning");
        }
        var $this = $(this);
    ...
    $(document).delegate("#reset", "click", function (e) {
        ...
        $("#paid-amount").val(0);
        $("#image_thumb img").attr("src", "../assets/pprtnaa/img/noimage.jpg");
        $("#image").val("");
    });
    ...
    $("#email-btn").on( "click", function (e) {
        ...
        var thehtml = dt.html();
        EmailModal({template: "default", subject: "Listado de facturas de compra", title:"Listado de facturas de compra", html: thehtml});
    });
}]);